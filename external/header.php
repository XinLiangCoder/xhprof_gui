...
if($_xhprof['ext_name'])
{
    ...
    $flagsnobuiltins = constant(strtoupper($_xhprof['ext_name']) . '_FLAGS_NO_BUILTINS');
}
...
if ($_xhprof['ext_name'] && $_xhprof['doprofile'] === true) {
    ...
    if (isset($ignoredFunctions) && is_array($ignoredFunctions) && !empty($ignoredFunctions)) {
        call_user_func($_xhprof['ext_name'] . '_enable', $flagsnobuiltins + $flagsCpu + $flagsMemory, array('ignored_functions' => $ignoredFunctions));
        //call_user_func($_xhprof['ext_name'].'_enable', $flagsCpu + $flagsMemory, array('ignored_functions' => $ignoredFunctions));
    } else {
        call_user_func($_xhprof['ext_name'].'_enable', $flagsCpu + $flagsMemory + $flagsnobuiltins);
    }
    unset($flagsCpu);
    unset($flagsMemory);
    unset($flagsnobuiltins);
